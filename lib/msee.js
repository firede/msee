    ...
    switch (type) {
        ...
        case 'list_item_start': {
            ...
            content = blockFormat(content, {
                ...
                pad_str: new Array(options.listItemPad.length).join(' '),
...
function blockFormat(src, opts) {
    ...
    var retLines = [];
    var secondWidth = opts.width - padStr.length;
    var firstWidth = opts.width - firstPadStr.length;
    var first = true;
    ...
    lines.forEach(function(rawLine) {
        for (var i = 0; i < rawLine.length;) {
            ...
            if (end > rawLine.length) {
                ...
                var lastSpace = rawLine.lastIndexOf(' ', end);
                if (lastSpace === -1) {
                    line = rawLine.substring(i, end);
                    i = end;
                } else {
                    line = rawLine.substring(i, lastSpace);
                    i = lastSpace + 1;
                }   
            }
            if (opts.block_color) {
